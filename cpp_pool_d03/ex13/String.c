...
** 
** Started on  Fri Jan  9 22:37:44 2015 Bertrand-Rapello Baptiste
** Last update Fri Jan  9 22:37:44 2015 Bertrand-Rapello Baptiste
*/
...
static String *split_s(String *this, char separator)
{
  ...
  while (this->str[c] != '\0')
    {
      if (this->str[c] == separator)
	d++;
      c++;
    }
  c = 0;
  d++;
  tab = malloc((d+1)*sizeof(String));
  tab[c++]->str =strtock(this->str, &separator);
  while (c < d)
    {
      tab[c] = strtok(NULL, &separator);
      c++;
    }
  tab[c] = NULL;
  return (tab); 
...
static char **split_c(String *this, char separator)
{
  ...
  c = 0;
  d++;
  tab = malloc((d+1)*sizeof(char*));
  tab[c++] = strtok(this->str, &separator);
  while (c < d)
    {
      tab[c] = strtok(NULL, &separator);
      c++;
